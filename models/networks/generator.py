...
class ASAPNetsGenerator(BaseNetwork):
    ...
    def __init__(self, opt, hr_stream=None, lr_stream=None, fast=False):
        ...
        num_inputs_lr = self.highres_stream.num_inputs
        self.lowres_stream = ASAPNetsLRStream(num_inputs_lr, num_params, width=opt.lr_width,
...
class ASAPNetsLRStream(th.nn.Sequential):
    ...
    def __init__(self, num_in, num_out, width=64, max_width=1024, depth=7, learned_ds_factor=16,
        ...
        model += [(th.nn.Conv2d(num_in, width, 3, stride=1, padding=padw)),
        ...
        #         model += [(th.nn.Conv2d(width, last_width, 3, stride=2, padding=padw)),
        #                   th.nn.ReLU(inplace=True)]
        #         width = last_width
        #     else:
        #         model += [(th.nn.Conv2d(width, width, 3, stride=2, padding=padw)),
        #               th.nn.ReLU(inplace=True)]

        # ConvNet to estimate the MLPs parameters"
        for i in range(count_ly, count_ly+depth):
            model += [ASAPNetsBlock(width, reflection_pad=reflection_pad, replicate_pad=replicate_pad)]
...
class ASAPNetsHRStream(th.nn.Module):
    ...
    def __init__(self, downsampling,
                 num_inputs, num_outputs, width=64, depth=5, coordinates="cosine"):
